...
const HomePage = observer(() => {
  ...
  // On mount (or when navigating to a different day): check if already played, otherwise load
  useEffect(() => {
    if (AppState.completedDateKeys.has(dateKey)) {
      navigate('/scores')
      return
    }
    AppState.resetGame()
    loadMangaForDate(gameDate)
  }, [dateKey, navigate]) // eslint-disable-line react-hooks/exhaustive-deps

  // When the game ends, save the score and redirect to the score card page.
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => navigate('/scores'), 2000)
    return () => clearTimeout(timer)
  }, [AppState.gameOver, dateKey, navigate]) // eslint-disable-line react-hooks/exhaustive-deps
